    ...
    def run_on_video(self, video):
        ...
        def process_predictions(frame, predictions):
            ...
            if k == 1:
                ...
                result = np.zeros((500,400,3))
                # x_scale = (result_width/vis_frame.shape[1])
                # y_scale = (result_height/vis_frame.shape[0])
                x_scale = (500/vis_frame.shape[1])
                y_scale = (400/vis_frame.shape[0])
                    ...
                    for box_j,track_j in person:
                        ...
                        if self.time_count % 10:
                            ...
                            for indexs,l in enumerate(self.all_track_id):
                                if l != objectid:
                                    self.disappear(l)
                                    if self.maximum_wait[l] >= 5:
                                        self.detrack(l,indexs)
                            
                        if box_i != box_j:
                            D = dist.euclidean(box_i,box_j)
                            xA,yA,zA = box_i
                            xB,yB,zB = box_j
